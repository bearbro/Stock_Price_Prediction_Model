...
def input_network(shape2, shape3, pathLength):
    ...
    sentiment_embedding_up = keras.layers.Embedding(input_dim=4, output_dim=50, name='sentiment_embedding',
    ...
    sentiment_score_embedding = keras.layers.Dense(units=50, use_bias=False, name='sentiment_score_embedding')(
        sentiment_score)  # bach,None,50

    a = b
    b += 1
    entity_score = keras.layers.Lambda(lambda xx: xx[:, :, a:b], name='entity_score')(my_input)  # bach,None,1
    entity_score_embedding = keras.layers.Dense(units=50, use_bias=False, name='entity_score_embedding')(
        entity_score)  # bach,None,50

    a = b
    b += 1
    link_score = keras.layers.Lambda(lambda xx: xx[:, :, a:b], name='link_score')(my_input)  # bach,None,1
    link_score_embedding = keras.layers.Dense(units=50, use_bias=False, name='link_score_embedding')(
        link_score)  # bach,None,50
    a = b
    b += 1
    link_count = keras.layers.Lambda(lambda xx: xx[:, :, a:b], name='link_count')(my_input)  # bach,None,1
    link_count_embedding = keras.layers.Dense(units=50, use_bias=False, name='link_count_embedding')(
        link_count)  # bach,None,50

    a = b
    b += 1
    stock_count = keras.layers.Lambda(lambda xx: xx[:, :, a:b], name='stock_count')(my_input)  # bach,None,1
    stock_count_embedding = keras.layers.Dense(units=50, use_bias=False, name='stock_count_embedding')(
        stock_count)  # bach,None,50

    a = b
    b += 3
    path = keras.layers.Lambda(lambda xx: xx[:, :, a:b], name='path')(my_input)  # bach,None,4
    path_embedding = keras.layers.Embedding(input_dim=22, output_dim=50, name='path_embedding', mask_zero=True)(
        path)  # bach,None,3,50
    ...
    path_embedding = MyGRUFor4Dims(input_shape=(shape2, pathLength, 50), units=50, use_bias=True,
...
def basic_network(shape2=None, shape3=None, pathLength=3):
...
if __name__ == '__main__':
    model = basic_network(shape2=10, shape3=768)
    model.build(input_shape=[None, 28, 150])
