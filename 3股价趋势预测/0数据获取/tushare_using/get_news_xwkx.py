...
def get_news(src='', start_date='', end_date=''):
    for xx in range(5):
        try:
            print('try %s %s ... %d' % (start_date,end_date,xx))
            logging.critical('try %s %s ... %d' % (start_date,end_date,xx))
            df=pro.news(src=src, start_date=start_date, end_date=end_date,fields='title,content,datetime,channels')
            ...
            time.sleep(config.xwkx_wait_time)
    error_img='error! %s'% '\t'.join([get_news.__name__, src, start_date, end_date])
    print(error_img)
...
def get_news_sub2(file_path, src, data_i, data_j):
    i = datetime.datetime.strftime(data_i, '%Y-%m-%d %H:%M:%S')#2018-11-21 09:00:00
    j = datetime.datetime.strftime(data_j, '%Y-%m-%d %H:%M:%S')
    df = get_news(src=src, start_date=i, end_date=j)
    df.to_csv(file_path,index=False,sep=config.sep,encoding='utf-8')
    df2=df.copy()
    while len(df2)==config.xwkx_io_max:
        time.sleep(config.xwkx_wait_time)
        data_j2=datetime.datetime.strptime(df2['datetime'].values[-1], '%Y-%m-%d %H:%M:%S')
        if j==datetime.datetime.strftime(data_j2, '%Y-%m-%d %H:%M:%S'):
            img='error! 1秒内新闻数大于io_max(%d)  for %s %s' % (config.xwkx_io_max,file_path,j)
            logging.error(img)
            print(img)
            # 手动减1秒
        ...
        df2 = get_news(src=src, start_date=i, end_date=j)
        # df=pd.concat([df,df2])
        df2.to_csv(file_path,index=False,sep=config.sep,encoding='utf-8',mode='a',header=False)
        if len(df2)>config.xwkx_io_max:
            logging.error('io_max 设置有误！当前 io_max=%d，收到数据个数=%d'%(config.xwkx_io_max,len(df2)))
            raise RuntimeError('io_max 设置有误！当前 io_max=%d，收到数据个数=%d'%(config.xwkx_io_max,len(df2)))
    df=pd.read_csv(file_path,sep=config.sep,encoding='utf-8')
    return df
    
   

for src in config.xwkx_news_scr:
    if not os.path.exists(os.path.join(config.xwkx_dir,src)):
        os.mkdir(os.path.join(config.xwkx_dir,src))
    start_date=config.xwkx_start_date
    end_date=config.xwkx_end_date
    ...
    while start_date!=end_date:
        ...
        file_path=os.path.join(config.xwkx_dir, src, '%s.csv'%(start_date))
        if not os.path.exists(file_path):
            
            df = get_news_sub2(file_path=file_path, src=src, data_i=data_i, data_j=data_j)
